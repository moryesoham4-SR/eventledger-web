...
function VendorsContent({ eventId }) {
  const { canApproveBudget: canManage, loading } = useMyRole(eventId) // event_admin/finance_head — same bar as managing budgets/income
  ...
  return (
    <SimpleResourcePage
      ...
      canCreate={canManage}
      canDelete={canManage}
      emptyHint={canManage ? undefined : 'No vendors yet. Ask an event admin or finance head to add one.'}
      renderItem={(item) => (
        <div>
          <p className="font-medium text-ink text-sm">
            {item.name}{' '}
            <span className="text-xs font-normal text-ink/40">· {item.category}</span>
          </p>
