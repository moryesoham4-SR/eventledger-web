...
function ProposalPanel({ proposalId, onChanged, role }) {
  ...
  const handleAddLine = async (e) => {
    ...
    try {
      ...
      setError(getErrorMessage(err, "Couldn't add that line item"))
    }
  }
  ...
  const handleAction = async (action) => {
    try {
      ...
      setError(getErrorMessage(err, `Couldn't ${action} this proposal`))
    }
  }

  if (!proposal)
    return <div className="h-48 bg-white border border-rule rounded-xl animate-pulse" />
  ...
  return (
    <div className="bg-white border border-rule rounded-xl p-5">
      ...
      {error && <div className="my-3 text-sm text-deficit-600 bg-deficit-50 rounded-lg px-3 py-2.5">{error}</div>}
      ...
      <div className="flex flex-wrap gap-2 mt-4 mb-4">
        ...
        {proposal.status === 'submitted' && canApprove && (
          <>
            ...
            <button onClick={() => handleAction('reject')} className="text-xs bg-deficit-500 text-white px-3.5 py-1.5 rounded-full font-semibold hover:bg-deficit-600 active:scale-95 transition-all">
              ✕ Reject
            </button>
          </>
        )}
        ...
        {proposal.status === 'draft' && canEdit && (
          <button onClick={() => setShowLineForm(!showLineForm)} className="text-xs border border-rule px-3.5 py-1.5 rounded-full font-semibold text-ink/70 hover:border-primary-400 hover:text-primary-600 transition-colors">
      ...
      {showLineForm && canEdit && (
        <form onSubmit={handleAddLine} className="bg-paper border border-rule rounded-xl p-4 mb-4 space-y-2">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <input placeholder="Category" required value={line.category} onChange={(e) => setLine({ ...line, category: e.target.value })} className="border border-rule rounded-lg px-2.5 py-1.5 text-sm bg-white" />
            <input placeholder="Item name" value={line.item_name} onChange={(e) => setLine({ ...line, item_name: e.target.value })} className="border border-rule rounded-lg px-2.5 py-1.5 text-sm bg-white" />
            <input placeholder="Unit (e.g. pcs)" value={line.unit} onChange={(e) => setLine({ ...line, unit: e.target.value })} className="border border-rule rounded-lg px-2.5 py-1.5 text-sm bg-white" />
            <input type="number" step="0.01" placeholder="Quantity" value={line.quantity} onChange={(e) => setLine({ ...line, quantity: e.target.value })} className="border border-rule rounded-lg px-2.5 py-1.5 text-sm bg-white" />
            <input type="number" step="0.01" placeholder="Unit price" required value={line.unit_price} onChange={(e) => setLine({ ...line, unit_price: e.target.value })} className="border border-rule rounded-lg px-2.5 py-1.5 text-sm bg-white" />
            <input placeholder="Description" value={line.description} onChange={(e) => setLine({ ...line, description: e.target.value })} className="border border-rule rounded-lg px-2.5 py-1.5 text-sm bg-white" />
          </div>
      ...
      {(proposal.line_items || []).length === 0 ? (
        ...
        <div className="border border-rule rounded-xl divide-y divide-rule overflow-hidden">
...
function BudgetContent({ eventId }) {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setError(getErrorMessage(err, "Couldn't create that proposal"))
    }
  }
    ...
    <div>
      ...
      {canCreateProposal && showForm && (
        <form onSubmit={handleCreate} className="bg-white border border-rule rounded-xl p-5 mb-6 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <input placeholder="Proposal title" required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="border border-rule rounded-lg px-3 py-2 text-sm" />
            <select
              required
              disabled={deptLocked}
              value={form.department_id}
              onChange={(e) => setForm({ ...form, department_id: e.target.value })}
              className="border border-rule rounded-lg px-3 py-2 text-sm disabled:bg-paper disabled:text-ink/50"
            >
            ...
            <input placeholder="Notes" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="border border-rule rounded-lg px-3 py-2 text-sm col-span-1 sm:col-span-2" />
          </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="h-40 bg-white border border-rule rounded-xl animate-pulse" />
          <div className="md:col-span-2 h-40 bg-white border border-rule rounded-xl animate-pulse" />
        </div>
      ) : proposals.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white border border-rule rounded-xl divide-y divide-rule h-fit overflow-hidden">
            {proposals.map((p) => (
              <button
                key={p.id}
                onClick={() => setSelectedId(p.id)}
                className={`w-full text-left px-4 py-3 hover:bg-paper transition-colors ${selectedId === p.id ? 'bg-primary-50' : ''}`}
              >
