...
function ProposalPanel({ proposalId, onChanged }) {
...
function BudgetContent({ eventId }) {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setForm({ department_id: '', title: '', notes: '' })
      setShowForm(false)
      await load()
  ...
  return (
    <div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-white border border-gray-200 rounded-lg p-5 mb-6 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <select required value={form.department_id} onChange={(e) => setForm({ ...form, department_id: e.target.value })} className="border border-gray-300 rounded px-3 py-2 text-sm">
      ...
      {loading ? (
        ...
        <p className="text-gray-500 text-sm">No budget proposals yet. Create one above — you'll need at least one department first.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2">
            {selectedId && <ProposalPanel proposalId={selectedId} onChanged={load} />}
          </div>
        </div>
      )}
    </div>
