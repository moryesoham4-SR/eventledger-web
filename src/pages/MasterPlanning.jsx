...
function MasterPlanningContent({ eventId }) {
  ...
  const [activeTab, setActiveTab] = useState('strategy') // 'strategy', 'budgets', 'risks'
  ...
  const loadData = async () => {
    ...
    setLoading(true)
    try {
      const [mpData, deptsList, propsList] = await Promise.all([
  ...
  const handleCreateRisk = async (e) => {
    ...
    try {
      ...
      toast.success('Risk item logged with Plan B mitigation!')
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-rule pb-4">
        <div>
          ...
          <p className="text-xs text-ink/60 mt-1">
            Executive War Room — Master Event Strategy, Plan A vs Plan B Contingency Budgets, and Risk Register.
          </p>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex border-b border-rule space-x-2">
        <button
          onClick={() => setActiveTab('strategy')}
          className={`px-4 py-2.5 text-xs font-bold font-display border-b-2 transition-all flex items-center gap-1.5 ${
            activeTab === 'strategy'
