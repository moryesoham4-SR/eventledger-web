...
export default function Users() {
  ...
  if (!currentUser?.is_super_admin) {
    return (
      <div className="bg-white border border-rule rounded-xl p-8 text-center">
  ...
  return (
    <div>
      ...
      <div className="flex bg-paper rounded-xl p-1 mb-6 w-fit">
        {['users', 'assign role', ...(currentUser?.is_super_admin ? ['reset password'] : []), 'audit log'].map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-1.5 rounded-full text-sm font-semibold capitalize transition-all ${tab === t ? 'bg-white shadow-sm text-ink' : 'text-ink/55'}`}
          >
            {t}
          </button>
        ))}
      ...
      {message && <div className="mb-4 text-sm text-primary-700 bg-primary-50 rounded px-3 py-2">{message}</div>}
      ...
      {loading ? (
        ...
        <form onSubmit={handleAssignRole} className="bg-white border border-rule rounded-xl p-5 space-y-3 max-w-md">
        ...
        <form onSubmit={handleResetPassword} className="bg-white border border-rule rounded-xl p-5 space-y-3 max-w-md">
