  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={handleMarkAllRead} className="text-sm text-primary-500 hover:text-primary-400 font-semibold">
          Mark all as read
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-card border border-rule rounded-xl divide-y divide-rule">
          {items.map((n) => (
            <div key={n.id} className={`flex items-start justify-between px-5 py-3 ${!n.is_read ? 'bg-primary-500/10' : ''}`}>
              <div>
                <p className="text-sm text-ink">{n.message || n.title || 'Notification'}</p>
                <p className="text-xs text-ink/40 mt-0.5">{n.created_at}</p>
              </div>
              {!n.is_read && (
                <button onClick={() => handleMarkRead(n.id)} className="text-xs text-primary-500 hover:text-primary-400 whitespace-nowrap ml-4">
                  Mark read
                </button>
              )}
            </div>
