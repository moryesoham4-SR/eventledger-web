...
import { EventCountdown, FinancialHealthWidget, EventProgress, DepartmentHealthCards, PendingApprovals, ActivityTimeline } from '../components/DashboardWidgets'
...
export default function Dashboard() {
  ...
  const { canApproveBudget } = useMyRole(activeEventId)
  ...
  return (
    <div>
      ...
      {eventsLoading ? (
        ...
        <div className="bg-card border border-rule rounded p-10 text-center">
          ...
          {loading || !summary || !widgetData ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-10">
                <PendingApprovals proposals={widgetData.proposals} currency={activeEvent.currency} canApprove={canApproveBudget} />
                <ActivityTimeline
                  proposals={widgetData.proposals}
                  actualExpenses={widgetData.actualExpenses}
      ...
      <div>
        ...
        {events.length > 0 && (
          <div className="bg-card border border-rule rounded divide-y divide-rule">
            {events.map((ev) => (
              <Link
                key={ev.id}
                to={`/events/${ev.id}`}
