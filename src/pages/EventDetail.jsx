...
export default function EventDetail() {
  ...
  const handleExportAll = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(err, "Couldn't export this event"))
    } finally {
      setExporting(false)
    }
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-card border border-rule rounded p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {SECTIONS.map((s) => (
              <Link
                ...
                to={s.path}
                onClick={() => setActiveEventId(String(id))}
                className="text-sm border border-rule rounded px-3 py-2 text-ink/75 hover:border-primary-400 hover:text-primary-500 transition-colors"
              >
                {s.label}
              </Link>
            ))}
      ...
      {canImportExport && (
        <div className="bg-card border border-rule rounded p-5 mt-6">
