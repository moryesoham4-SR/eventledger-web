...
export default function EventDetail() {
  ...
  const { canManageDepartments } = useMyRole(id) // event_admin-only powers, reused here for the status toggle
          ...
          {canManageDepartments && (
            <button
              ...
              className={`text-sm px-4 py-2 rounded-full font-semibold shadow-sm transition-transform active:scale-95 disabled:opacity-50 ${
                isFinished
                  ? 'bg-white border border-ink/20 text-ink hover:border-ink/40'
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {SECTIONS.map((s) => (
              <Link
                ...
                to={s.path}
                onClick={() => setActiveEventId(String(id))}
                className="text-sm border border-rule rounded px-3 py-2 text-ink/75 hover:border-primary-400 hover:text-primary-600 transition-colors"
              >
                {s.label}
              </Link>
            ))}
