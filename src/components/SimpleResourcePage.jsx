    ...
    <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {fields.map((f) => (
              <input
                ...
                className="border border-rule rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/40 focus:border-primary-500 transition-colors"
              />
            ))}
          </div>
          <button
      ...
      {loading ? (
        ...
        <div className="bg-white border border-rule rounded-xl divide-y divide-rule">
          {items.map((item) => (
            <div key={item.id} className="flex items-center justify-between px-5 py-3.5 hover:bg-paper/60 transition-colors">
